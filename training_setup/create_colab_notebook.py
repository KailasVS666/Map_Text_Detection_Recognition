...
code([
    ...
    "# Regex approach: strip any previous attempts, then re-insert cleanly",
    "import re as _re",
    "iou_path = os.path.join(PADDLEOCR_DIR, 'ppocr/metrics/eval_det_iou.py')",
    "with open(iou_path, 'r') as f: _iou = f.read()",
    "# Strip all previously injected hasattr/reshape lines (any indentation)",
    "_iou = _re.sub(r'[ \\t]+if hasattr\\(points.*?ndim.*?\\n', '', _iou)",
    "_iou = _re.sub(r'[ \\t]+points = points\\.reshape\\(-1, 2\\)\\n', '', _iou)",
    "# Insert guard before Polygon validity check, auto-detecting indent",
    "def _guard(m):",
    "    sp = m.group(1)",
    "    return (f'{sp}if hasattr(points, \"reshape\") and points.ndim == 1:\\n'",
    "            f'{sp}    points = points.reshape(-1, 2)\\n'",
    "            f'{sp}if not Polygon(points).is_valid:')",
    "_iou2, _n = _re.subn(r'^([ \\t]+)if not Polygon\\(points\\)\\.is_valid:', _guard, _iou, flags=_re.MULTILINE)",
    "with open(iou_path, 'w') as f: f.write(_iou2)",
    "print(f'✓ Patched eval_det_iou.py ({_n} site(s) fixed)' if _n else '✓ eval_det_iou.py already patched')",
    "",
