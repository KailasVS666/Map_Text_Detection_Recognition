...
code([
    ...
    "# Line-number approach: find Polygon line, strip surrounding broken lines, insert correct block",
    ...
    "_correct = ('            if hasattr(points, \"reshape\") and points.ndim == 1:\\n'",
    "            '                points = points.reshape(-1, 2)\\n'",
    "            '            if not Polygon(points).is_valid:\\n'",
    "            '                continue\\n')",
    "if _correct in ''.join(_lines):",
    "    print('✓ eval_det_iou.py already correctly patched')",
    "else:",
    "    # Find Polygon validity check line",
    "    _idx = next((i for i, l in enumerate(_lines) if 'Polygon(points).is_valid' in l), None)",
    "    assert _idx, 'Could not find Polygon validity line in eval_det_iou.py'",
    "    # Strip any broken hasattr/reshape lines immediately before it",
    "    while _idx > 0 and ('hasattr(points' in _lines[_idx-1] or 'points.reshape(-1, 2)' in _lines[_idx-1]):",
    "        _lines.pop(_idx - 1); _idx -= 1",
    "    # Get correct indentation from dontCare reference line",
    "    _sp = next(((' ' * (len(l) - len(l.lstrip()))) for l in _lines if 'dontCare = gt[n]' in l), '            ')",
    "    # Remove the Polygon line and its continue",
    "    _lines.pop(_idx)",
    "    if _idx < len(_lines) and 'continue' in _lines[_idx]: _lines.pop(_idx)",
    "    # Insert correct block",
    "    for _j, _l in enumerate([",
    "        f'{_sp}if hasattr(points, \"reshape\") and points.ndim == 1:\\n',",
    "        f'{_sp}    points = points.reshape(-1, 2)\\n',",
    "        f'{_sp}if not Polygon(points).is_valid:\\n',",
    "        f'{_sp}    continue\\n',",
    "    ]): _lines.insert(_idx + _j, _l)",
    "    with open(iou_path, 'w') as f: f.writelines(_lines)",
    "    print('✓ Patched eval_det_iou.py (Shapely 2.x)')",
    "",
    ...
    "        t['DetResizeForTest'] = {'limit_type': 'resize', 'limit_side_len': 960}",
    "        break",
    "with open(CONFIG, 'w') as f: yaml.safe_dump(cfg, f, sort_keys=False)",
    "print('✓ Config verified (eval_batch_step=50, DetResizeForTest fixed)')",
    "",
