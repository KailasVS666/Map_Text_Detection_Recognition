...
code([
    ...
    "# Patches ALL occurrences of Polygon validity check (gt loop + det loop)",
    "iou_path = os.path.join(PADDLEOCR_DIR, 'ppocr/metrics/eval_det_iou.py')",
    "with open(iou_path, 'r') as f: _lines = f.readlines()",
    "_fixed = 0",
    "_i = 0",
    "while _i < len(_lines):",
    "    if 'Polygon(points).is_valid' in _lines[_i] and 'hasattr' not in _lines[_i]:",
    "        # Skip if reshape guard already present on line before",
    "        if _i > 0 and 'points.reshape(-1, 2)' in _lines[_i - 1]:",
    "            _i += 1; continue",
    "        _sp = ' ' * (len(_lines[_i]) - len(_lines[_i].lstrip()))",
    "        _lines.insert(_i,     f'{_sp}if hasattr(points, \"reshape\") and points.ndim == 1:\\n')",
    "        _lines.insert(_i + 1, f'{_sp}    points = points.reshape(-1, 2)\\n')",
    "        _fixed += 1",
    "        _i += 3",
    "    else:",
    "        _i += 1",
    "with open(iou_path, 'w') as f: f.writelines(_lines)",
    "print(f'✓ Patched eval_det_iou.py ({_fixed} site(s))' if _fixed else '✓ eval_det_iou.py already patched')",
    "",
