...
code([
    ...
    "with open(iou_path, 'r') as f: iou_code = f.read()",
    "_old2 = '        if not Polygon(points).is_valid:'",
    "_new2 = ('        if hasattr(points, \"reshape\") and points.ndim == 1:\\n'",
    "         '            points = points.reshape(-1, 2)\\n'",
    "         '        if not Polygon(points).is_valid:')",
    "if _old2 in iou_code and _new2 not in iou_code:",
    "    iou_code = iou_code.replace(_old2, _new2)",
    "    with open(iou_path, 'w') as f: f.write(iou_code)",
    "    print('✓ Patched eval_det_iou.py (Shapely 2.x fix)')",
    "else:",
    "    print('✓ eval_det_iou.py already patched or not needed')",
    "",
