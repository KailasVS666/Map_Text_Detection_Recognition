...
code([
    ...
    "# Uses exact-string replacement — handles original file and any broken state",
    "iou_path = os.path.join(PADDLEOCR_DIR, 'ppocr/metrics/eval_det_iou.py')",
    "with open(iou_path, 'r') as f: _iou = f.read()",
    "# Target: the correct final state (12-space indent throughout)",
    "_correct = ('            if hasattr(points, \"reshape\") and points.ndim == 1:\\n'",
    "            '                points = points.reshape(-1, 2)\\n'",
    "            '            if not Polygon(points).is_valid:\\n'",
    "            '                continue\\n')",
    "# All known broken/original variants to replace",
    "_variants = [",
    "    # Original unpatched file",
    "    '            if not Polygon(points).is_valid:\\n                continue\\n',",
    "    # 8-space indent variant (from regex mismatch)",
    "    '        if hasattr(points, \"reshape\") and points.ndim == 1:\\n            points = points.reshape(-1, 2)\\n        if not Polygon(points).is_valid:\\n                continue\\n',",
    "    # Duplicate hasattr + broken continue",
    "    '            if hasattr(points, \"reshape\") and points.ndim == 1:\\n                points = points.reshape(-1, 2)\\n            if hasattr(points, \"reshape\") and points.ndim == 1:\\n            points = points.reshape(-1, 2)\\n        if not Polygon(points).is_valid:\\n                continue\\n',",
    "]",
    "_patched = False",
    "for _v in _variants:",
    "    if _v in _iou:",
    "        _iou = _iou.replace(_v, _correct)",
    "        _patched = True",
    "        break",
    "with open(iou_path, 'w') as f: f.write(_iou)",
    "print('✓ Patched eval_det_iou.py (Shapely 2.x)' if _patched else '✓ eval_det_iou.py already patched')",
    "",
