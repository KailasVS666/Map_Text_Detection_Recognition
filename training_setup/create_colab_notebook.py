...
code([
    ...
    "# Nuke all previous injected lines, then re-insert cleanly at all Polygon sites",
    "iou_path = os.path.join(PADDLEOCR_DIR, 'ppocr/metrics/eval_det_iou.py')",
    "with open(iou_path, 'r') as f: _lines = f.readlines()",
    "# Step 1: remove all previously injected hasattr/reshape lines (any indent)",
    "_lines = [l for l in _lines if 'hasattr(points' not in l and 'points.reshape(-1, 2)' not in l]",
    "# Step 2: insert fresh guard before every Polygon validity check",
    "_out = []",
    "_fixed = 0",
    "for _l in _lines:",
    "    if 'Polygon(points).is_valid' in _l:",
    "        _sp = ' ' * (len(_l) - len(_l.lstrip()))",
    "        _out.append(f'{_sp}if hasattr(points, \"reshape\") and points.ndim == 1:\\n')",
    "        _out.append(f'{_sp}    points = points.reshape(-1, 2)\\n')",
    "        _fixed += 1",
    "    _out.append(_l)",
