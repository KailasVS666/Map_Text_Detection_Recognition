...
code([
    ...
    "# det_metric.py (2.6.1) passes 1 dict per IMAGE with points=(N,4,2), ignore=(N,)",
    "# instead of N dicts per polygon. Fix by unwrapping at evaluation time.",
    "with open(iou_path, 'r') as f: _lines = f.readlines()",
    "_NORM = [",
    "    '        import numpy as _np\\n',",
    "    '        _gt_norm = []\\n',",
    ...
    "# cal_metric_during_train=True causes db_postprocess to receive the training",
    "# threshold_map (shape BxHxW) as shape_list instead of actual img shapes -> crash.",
    "# eval_batch_step already runs proper val-set evals every 50 steps.",
    ...
    "",
    "# ── 5. Patch det_basic_loss.py: fully NaN-proof (handles any prior patch state) ──",
    "# Root cause: MakeShrinkMap/MakeBorderMap emits NaN for degenerate polygons.",
    "# Tensor method .nan_to_num() is unreliable in Paddle 2.6.1 — use the module",
    "# function paddle.nan_to_num() instead, and .numpy() for safe int() conversion.",
    "# This patch handles both the original file AND any partially-patched state.",
    "_bl_path = os.path.join(PADDLEOCR_DIR, 'ppocr/losses/det_basic_loss.py')",
    "with open(_bl_path, 'r') as f: _bl = f.read()",
    "_changed = 0",
    "_SAFE_POS   = 'positive = paddle.nan_to_num(gt * mask, nan=0.0)'",
    "_SAFE_NEG   = 'negative = paddle.nan_to_num((1 - gt) * mask, nan=0.0)'",
    "_SAFE_PCNT  = 'positive_count = int(float(paddle.nan_to_num(positive, nan=0.0).sum().numpy()))'",
    "_SAFE_NCNT  = 'negative_count = int(min(float(paddle.nan_to_num(negative, nan=0.0).sum().numpy()),'",
    "_SAFE_CLIP  = 'loss = paddle.clip(loss, max=1.0)'",
    "# All known variants of each line (original + previous partial patches)",
    "for _old, _new in [",
    "    ('positive = gt * mask',                              _SAFE_POS),",
    "    ('positive = (gt * mask).nan_to_num(nan=0.0)',        _SAFE_POS),",
    "    ('negative = (1 - gt) * mask',                        _SAFE_NEG),",
    "    ('negative = ((1 - gt) * mask).nan_to_num(nan=0.0)',  _SAFE_NEG),",
    "    ('positive_count = int(positive.sum())',               _SAFE_PCNT),",
    "    ('positive_count = int(positive.nan_to_num(nan=0.0).sum())', _SAFE_PCNT),",
    "    ('negative_count = int(min(negative.sum(),',           _SAFE_NCNT),",
    "    ('negative_count = int(min(negative.nan_to_num(nan=0.0).sum(),', _SAFE_NCNT),",
    "    ('assert loss <= 1',                                   _SAFE_CLIP),",
    "]:",
    "    if _old in _bl and _new not in _bl:",
    "        _bl = _bl.replace(_old, _new); _changed += 1",
    "with open(_bl_path, 'w') as f: f.write(_bl)",
    "print(f'✓ det_basic_loss.py: {_changed} fixes applied' if _changed else '✓ det_basic_loss.py already fully patched')",
    "",
    "# ── 6. Auto-resume or fresh start ──",
