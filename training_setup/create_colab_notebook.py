...
code([
    ...
    "",
    "# ── 5. Patch det_basic_loss.py: NaN-safe OHEM + remove fragile assertions ──",
    "# (a) BalanceLoss: NaN from degenerate polygons breaks int(positive.sum())",
    "# (b) DiceLoss: old code has `assert loss <= 1` which fires when gt/pred",
    "#     contain NaN → nan comparison returns False → assertion triggers.",
    "#     Fix: replace with paddle.clip(loss, max=1.0) matching the newer upstream.",
    "_bl_path = os.path.join(PADDLEOCR_DIR, 'ppocr/losses/det_basic_loss.py')",
    "with open(_bl_path, 'r') as f: _bl = f.read()",
    "_changed = 0",
    "# 5a: nan_to_num on BalanceLoss positive/negative mask computation",
    "for _old, _new in [",
    "    ('positive = gt * mask',       'positive = (gt * mask).nan_to_num(nan=0.0)'),",
    "    ('negative = (1 - gt) * mask', 'negative = ((1 - gt) * mask).nan_to_num(nan=0.0)'),",
    "]:",
    "    if _old in _bl:",
    "        _bl = _bl.replace(_old, _new); _changed += 1",
    "# 5b: NaN-safe int() cast on BalanceLoss counts",
    "for _old, _new in [",
    "    ('positive_count = int(positive.sum())',",
    "     'positive_count = int(positive.nan_to_num(nan=0.0).sum())'),",
    "    ('negative_count = int(min(negative.sum(),',",
    "     'negative_count = int(min(negative.nan_to_num(nan=0.0).sum(),'),",
    "]:",
    "    if _old in _bl and _new not in _bl:",
    "        _bl = _bl.replace(_old, _new); _changed += 1",
    "# 5c: DiceLoss — replace `assert loss <= 1` with clamp (upstream fix)",
    "for _old, _new in [",
    "    ('assert loss <= 1', 'loss = paddle.clip(loss, max=1.0)'),",
    "]:",
    "    if _old in _bl:",
    "        _bl = _bl.replace(_old, _new); _changed += 1",
    "with open(_bl_path, 'w') as f: f.write(_bl)",
    "print(f'✓ Patched det_basic_loss.py ({_changed} fixes applied)' if _changed else '✓ det_basic_loss.py already patched')",
    "",
