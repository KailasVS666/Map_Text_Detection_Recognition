...
code([
    ...
    "# Uses line-by-line rewrite to get indentation right regardless of file state",
    "iou_path = os.path.join(PADDLEOCR_DIR, 'ppocr/metrics/eval_det_iou.py')",
    "with open(iou_path, 'r') as f: orig_lines = f.readlines()",
    "new_lines = []",
    "patched = False",
    "i = 0",
    "while i < len(orig_lines):",
    "    line = orig_lines[i]",
    "    # Remove any previously broken hasattr lines",
    "    if 'hasattr(points' in line and 'ndim' in line:",
    "        i += 1",
    "        continue",
    "    if 'points.reshape(-1, 2)' in line and 'if ' not in line:",
    "        i += 1",
    "        continue",
    "    # Insert reshape guard just before the Polygon validity check",
    "    if 'Polygon(points).is_valid' in line and 'hasattr' not in line:",
    "        sp = ' ' * (len(line) - len(line.lstrip()))",
    "        new_lines.append(f'{sp}if hasattr(points, \"reshape\") and points.ndim == 1:\\n')",
    "        new_lines.append(f'{sp}    points = points.reshape(-1, 2)\\n')",
    "        patched = True",
    "    new_lines.append(line)",
    "    i += 1",
    "with open(iou_path, 'w') as f: f.writelines(new_lines)",
    "print('✓ Patched eval_det_iou.py (Shapely 2.x)' if patched else '✓ eval_det_iou.py already patched')",
    "",
