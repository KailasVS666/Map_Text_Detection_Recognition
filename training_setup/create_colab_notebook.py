...
code([
    ...
    "",
    "# ── 3. Patch eval_det_iou.py: dontCare arrives as numpy array, cast to bool ──",
    "with open(iou_path, 'r') as f: _code = f.read()",
    "_old_dc = 'dontCare = gt[n][\"ignore\"]'",
    "_new_dc = 'dontCare = bool(gt[n][\"ignore\"])'",
    "if _old_dc in _code:",
    "    _code = _code.replace(_old_dc, _new_dc)",
    "    with open(iou_path, 'w') as f: f.write(_code)",
    "    print('✓ eval_det_iou.py dontCare → bool() patched')",
    "else:",
    "    print('✓ dontCare patch not needed (already fixed)')",
    "",
